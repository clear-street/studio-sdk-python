...
from studio_sdk import StudioSDK, AsyncStudioSDK, DefaultAioHttpClient
from studio_sdk._utils import is_dict
...
def client(request: FixtureRequest) -> Iterator[StudioSDK]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with StudioSDK(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncStudioSDK]:
