...
class PositionsResource(SyncAPIResource):
    ...
    def retrieve(
        ...
        *,
        ...
        return self._get(
            path_template("/accounts/{account_id}/positions/{symbol}", account_id=account_id, symbol=symbol),
    ...
    def list(
        ...
        *,
        ...
        if not account_id:
            raise ValueError(f"Expected a non-empty value for `account_id` but received {account_id!r}")
        return self._get(
            path_template("/accounts/{account_id}/positions", account_id=account_id),
            options=make_request_options(
